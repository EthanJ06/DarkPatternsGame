...
const VARIANTS = [
  {
    options: [
      ...
      { label: "Enable personalised ads", trap: true, checked: true },
      { label: "Opt out of all data collection", trap: false, checked: false },
    ],
  },
  {
    options: [
      { label: "Allow partner data sharing", trap: true, checked: true },
      { label: "Enable cross-site tracking", trap: true, checked: false },
      { label: "Global privacy control", trap: false, checked: false },
    ],
  },
  {
    options: [
      { label: "I consent to data sharing", trap: true, checked: true },
      { label: "Allow ad personalisation", trap: true, checked: false },
      ...
      el.insertAdjacentHTML("beforeend", `
        <div style="overflow-y:auto;min-height:0">
          ...
          <div class="inline-note" style="background:#EEEDFE;color:#26215C">
            NexusAI has re-optimized this form (variant ${round + 1}). <em>One option always opts you out — find it. One shot per round.</em>
          </div>
